class TaskControl extends BaseHTMLElement {
  ...
  init() {
    ...
    this.dom.application_list.addEventListener('click', (e: MouseEvent) => {
      ...
      if (application_item) {
        let application = this.application_list.find((i: Application) => i.id === application_item.id)
  ...
  createApplication(application: Application) {
    this.current_application = this.application_list.find((i: Application) => i.id === application.id)
  ...
  readApplication(application: Application) {
    this.current_application = application
    alert('读取应用')
  }
  ...
  deleteApplication(application: Application) {
    this.current_application = application
    this.current_application_dom = this.application_dom_list.find((i: HTMLElement) => i.id === application.id)
    if(!this.current_application_dom) return
  ...
  highlightApplication(application: Application) {
    this.current_application = application
    this.current_application_dom = this.getCurrentApplicationDom()
    this.current_application_dom.classList.add('highlight')
    setTimeout(() => {
      this.application_dom_list.forEach((i: HTMLElement) => i.classList.remove('highlight'))
    }, 1000)
  }

  getCurrentApplicationDom() {
    return this.application_dom_list.find((i: HTMLElement) => i.id === this.current_application.id)
  }
}
