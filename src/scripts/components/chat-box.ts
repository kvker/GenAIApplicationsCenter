class ChatBox extends BaseHTMLElement {
  ...
  updateAnswer(text: string, done = false, idx?: number) {
  ...
  requestChat() {
    ...
    chat.sse(text_list, (value: string, done: boolean) => {
      if (done) {
  ...
  getChatContext() {
    let children = (Array.from(this.chat_box.children) as HTMLElement[]).map((node: HTMLElement) => node.innerText)
  ...
  dispatchCustomEvent(event_name: string, value: any) {
    // 创建自定义事件
    let customEvent
    switch (event_name) {
      default:
